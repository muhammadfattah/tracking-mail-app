        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover table-sm text-center" id="dataTable">
                    ...
                    <tbody class="tbody-table table-surat-masuk" data-url="{{ url('pranata/datatable') }}">
                        @if (count($mails) == 0)
                            <tr>
                                ...
                                <tr class="text-dark">
                                    ...
                                    <td class="text-nowrap align-middle">
                                        <button class="btn btn-primary btn-sm tombol-detail penerima"
                                            data-url="{{ url('pranata', [$mail->id]) }}"
                                            data-from_id="{{ $mail->from_id }}" data-is_read="{{ $mail->is_read }}"><i
                                                class="fa fa-fw fa-info-circle"></i>
                                            Detail</button>
                                        <a href="{{ url('pranata/kirim', [$mail->id]) }}" class=" btn btn-info btn-sm"><i
                                                class="fas fa-fw fa-file-import"></i> Kirim</a>
                                        <form action="{{ url('pranata', [$mail->id, 'penerima']) }}" method="POST"
